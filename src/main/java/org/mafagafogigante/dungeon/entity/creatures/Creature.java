...
public class Creature extends Entity {
	...
	/**
	 * What caused the death of this creature. If getHealth().isAlive() evaluates to
	 * true, this should be null.
	 */
	private CauseOfDeath causeOfDeath;

	/**
	 * Constructs a Creature from the provided CreaturePreset. Most of the
	 * application code should obtain new creatures from the CreatureFactory.
	 */
		...
		if (condition.getEffect().getMaximumStack() > 0) {
			...
			for (Condition existingCondition : conditions) {
				if (existingCondition.equals(condition)
						&& existingCondition.getEffect().equals(condition.getEffect())) {
					Date existingConditionExpiration = existingCondition.getExpirationDate();
					if (firstToExpire == null
							|| existingConditionExpiration.compareTo(firstToExpire.getExpirationDate()) < 0) {
	...
	/**
	 * Returns the attack of this creature after taking into account all of its
	 * active conditions.
	 */
	int getAttack() {
		int total = attack;
	...
	/**
	 * Returns the fishing proficiency of this creature after taking into account
	 * all of its active conditions.
	 */
	...
	/**
	 * Sets an Item as the currently equipped weapon. The Item must be in this
	 * Creature's inventory and have the WEAPON tag.
	 *
	 * @param weapon an Item that must be in this Creature's inventory and have the
	 *               WEAPON tag
	 */
	public void setWeapon(Item weapon) {
		if (inventory.hasItem(weapon)) {
			if (weapon.hasTag(Item.Tag.WEAPON)) {
				...
				DungeonLogger.warning(
						String.format("Tried to equip %s (no WEAPON tag) on %s.", weapon.getName(), getName()));
			}
		} else {
	...
	/**
	 * Retrieves what caused the death of this creature. If getHealth().isAlive()
	 * evaluates to true, this method returns null.
	 */
	...
	/**
	 * Sets what caused the death of this creature. Should be called only once and
	...
	/**
	 * Returns a List with all the items this Creature dropped when it died. If this
	 * Creature is still alive, returns an empty list.
	 *
	...
	/**
	 * Checks if the Hero can see a given Entity based on the luminosity of the
	 * Location the Hero is in and on the visibility of the specified Entity.
	 */
	...
	public <T extends Entity> List<T> filterByVisibility(List<T> list) {
