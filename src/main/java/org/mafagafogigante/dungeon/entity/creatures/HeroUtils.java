...
public final class HeroUtils {
	 ...
	 * @return a boolean indicating if all Entities in the collection have the same
	 *         name
	 */
	public static boolean checkIfAllEntitiesHaveTheSameName(Collection<? extends Entity> entities) {
	 ...
	 *
	 * @param entities a {@code Collection} of Entities
	 * @param ignored  an Entity to be ignored, should be {@code null} if no Entity
	 *                 is to be ignored
	 * @return a boolean indicating if all Entities in the collection have the same
	 *         name
	 */
	...
	public static List<Item> findItems(List<Item> items, String[] tokens) {
		...
		if (matches.size() == 0) {
			...
			// Matches are disjoint, if there is more than one different name, the results
			// are ambiguous.
	...
	public static void writeNoLongerInInventoryMessage(Item item) {
