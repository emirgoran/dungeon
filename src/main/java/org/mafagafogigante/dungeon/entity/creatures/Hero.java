...
import org.mafagafogigante.dungeon.entity.HeroInventoryHandler;
...
public class Hero extends Creature {
	...
	// The longest possible sleep starts at 19:00 and ends at 05:15 (takes 10 hours
	// and 15 minutes).
	...
	private static final Effect WELL_FED_EFFECT = EffectFactory.getDefaultFactory().getEffect(WELL_FED_ID,
			WELL_FED_LIST);
	...
	private HeroInventoryHandler inventoryHandler = HeroInventoryHandler.getInstance(this);
	...
	/**
	 * Increments the Hero's health by a certain amount, without exceeding its
	 * maximum health. If at the end the Hero is completely healed, a messaging
	 * about this is written.
	 */
	...
	public void rest() {
		...
		if (getHealth().getCurrent() >= maximumHealthFromRest) {
			...
			// The fraction SECONDS_TO_REGENERATE_FULL_HEALTH / getHealth().getMaximum() may
			// be smaller than 1.
			int timeResting = Math.max(1,
					healthRecovered * SECONDS_TO_REGENERATE_FULL_HEALTH / getHealth().getMaximum());
	 ...
	 *
	 * <p>
	 * Depending on how much the Hero will sleep, this method may print a few
	 * dreams.
	 */
	public void sleep() {
		int seconds;
		...
		if (pod == PartOfDay.EVENING || pod == PartOfDay.MIDNIGHT || pod == PartOfDay.NIGHT) {
			...
			// In order to increase realism, add some time for the time it would take to
			// wake up exactly at dawn.
			...
			while (seconds > 0) {
				...
				// Cast to long because it is considered best practice. We are going to end with
				// a long anyway, so start doing
				// long arithmetic at the first multiplication. Reported by
				// ICAST_INTEGER_MULTIPLY_CAST_TO_LONG in FindBugs.
	...
	/**
	 * Prints the name of the player's current location and lists all creatures and
	 * items the character sees.
	 */
	public void look() {
		observer.look();
	}
	...
	/**
	 * Select a list of items of the current location based on the arguments of a
	 * command.
	 */
	...
	/**
	 * Selects items of the specified {@code BaseInventory} based on the arguments
	 * of a command.
	 *
	 * @param arguments          an array of arguments that will determine the item
	 *                           search
	...
	/**
	 * Attempts to find a creature in the current location comparing its name to an
	 ...
	 *
	 * <p>
	 * If there are multiple matches but all have the same name, the first one is
	 * returned.
	 *
	 * <p>
	 * If there are multiple matches with only two different names and one of these
	 * names is the Hero's name, the first creature match is returned.
	 *
	 * <p>
	 * Lastly, if there are multiple matches that do not fall in one of the two
	 * categories above, {@code null} is returned.
	 *
	...
	public void pickItems(String[] arguments) {
		if (canSeeAnItem()) {
			...
			for (Item item : selectedItems) {
				if (inventoryHandler.pickUpItem(item)) {
	...
	/**
	 * Adds an Item object to the inventory. As a precondition,
	 * simulateItemAddition(Item) should return SUCCESSFUL.
	 *
	 * <p>
	...
	public void addItem(Item item) {
		inventoryHandler.addItemToInventory(item);
	}
	...
	public void dropItems(String[] arguments) {
		List<Item> selectedItems = inventoryHandler.selectItems(arguments, false);
		for (Item item : selectedItems) {
			if (item == getWeapon()) {
				unsetWeapon(); // Just unset the weapon, it does not need to be moved to the inventory before
								// being dropped.
			}
			...
			// The character "dropped" the item even if it disappeared while doing it, so
			// write about it.
	...
	/**
	 * Prints all items in the Hero's inventory. This function should only be called
	...
	/**
	 * Attempts to learn a spell from a BookComponent object. As a precondition,
	 * book must be didactic (teach a spell).
	 *
	...
	private void destroyItem(@NotNull Item target) {
		...
		if (target.isBroken()) {
			...
			while (getLocation().getInventory().hasItem(target) && !target.isBroken()) {
				// Simulate item-on-item damage by decrementing an item's integrity by its own
				// hit decrement.
	...
	public void readTime() {
		...
		if (clock != null) {
			...
			// Assume that the hero takes the same time to read the clock and to put it back
			// where it was.
	 ...
	 * not have a clock at all, null is returned.
	 *
